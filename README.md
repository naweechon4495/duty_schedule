...
- ผูกบัญชีผู้ใช้กับพยาบาลด้วยรหัส
- ล็อกอิน username/password และ (ตัวเลือก) LINE Login
- 3 บทบาท: admin / approver / requester

> ⚠️ **หลังอัปเดตนี้ต้อง redeploy Apps Script** (`apps_script_backend.gs`) หนึ่งครั้ง เพราะเพิ่มตาราง `Leaves` และคอลัมน์ `type` (Swaps) / `nurseCode` (Users) — ก่อน redeploy แอปยังใช้งานได้จาก localStorage แต่ข้อมูลใหม่จะยังไม่ sync ขึ้น Google Sheet
